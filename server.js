...
// helper: get latest version id by slug
async function getLatestVersionId(slug) {
  const url = `https://api.replicate.com/v1/models/${slug}`;
  const r = await fetch(url, {
    headers: { Authorization: `Token ${process.env.REPLICATE_API_TOKEN}` },
  });
  if (!r.ok) {
    const t = await r.text().catch(() => "");
    throw new Error(`getLatestVersionId failed ${r.status}: ${t}`);
  }
  const data = await r.json();
  const vid = data?.latest_version?.id || data?.versions?.[0]?.id;
  if (!vid) throw new Error(`No version id for ${slug}`);
  return vid;
}
...
app.post("/api/image-studio", async (req, res) => {
  try {
    ...
    const orbitAngles = (count) => {
      ...
      return Array.from({ length: count }, (_, i) => order[i % order.length]).map(
        (k) => ANGLE_MAP[k] || k
      );
    };
    ...
    async function waitPrediction(id, model, maxMs = MAX_WAIT_MS) {
      ...
      while (true) {
        ...
        if (p.status === "failed" || p.status === "canceled")
          throw new Error(`prediction ${p.status} (${model}) ${p.error ? `: ${p.error}` : ""}`);
      ...
      if (action === "text2img") {
        ...
        for (const m of MODELS) {
          try {
            ...
            const pred = await createPredictionBySlug(m.slug, inputX);
...
app.post("/api/video-studio", async (req, res) => {
  try {
    ...
    const buildVPrompt = ({
      ...
      const topic = [category || "General", subcategory || ""]
        .filter(Boolean)
        .join(" • ");
      const hints = {
      ...
      try {
        if (versionI2V) {
          ...
          const job = await fetchJson("https://api.replicate.com/v1/predictions", {
            method: "POST",
            headers: REPLICATE_HEADERS,
            body: JSON.stringify({ version: versionId, input: inputI2V }),
          });
    ...
    if (text_only) {
      ...
      if (versionImg) {
        ...
        try {
          ...
          image_url =
            Array.isArray(outI) ? outI[0] : typeof outI === "string" ? outI : null;
        } catch (e) {}
      }
    } else if (full_mode) {
