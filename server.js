...
// ========================================================
// PAYWALL: extractKey → getRecord → key-check → guardPaid
// ========================================================

// Вытащить ключ из запроса
...
// Нормализовать запись ключа
function getKeyRecord(key) {
  const all = loadKeys();             // ← всегда читаем свежий файл
  const rec = all[key];
  if (!rec) return null;
  ...
  return { key, limit, used, raw: rec, all };
}

// Проверка ключа (без списания)
...
// Middleware списывает 1 генерацию
function guardPaid(req, res, next) {
  try {
    ...
    rec.all[rec.key] = rec.raw;

    saveKeys(rec.all);   // ← сохраняем keys.json
