...
app.post("/api/shorten", async (req, res) => {
  try {
    ...
    for (const [k,v] of Object.entries(SHORT_MAP)) if (v === long) return res.json({ ok:true, short: absUrl(req, `/s/${k}`) });
      ...
      try {  
        ...
        async function trySDXLByVersion() {  
          ...
          const job = await replicatePredict(process.env.REPLICATE_MODEL_VERSION_SDXL, {  
            prompt: vprompt, negative_prompt: negative, width: [896,1600,1280,720,1024,1024] && undefined, height: undefined, num_inference_steps: 30, guidance_scale: 7.0, scheduler: "DPMSolverMultistep", num_outputs: 1,  
          });  
          return Array.isArray(job.output) ? job.output[0] : job.output;  
        }  
...
app.post("/api/image-studio", async (req, res) => {  
  try {  
    ...
    const ANGLE_MAP = {  
      front: "front view", "34left": "3/4 angle, left side", "34right": "3/4 angle, right side", side: "side view", back: "back view", top: "top-down view", low: "low-angle cinematic shot",  
    };  
...
app.post("/api/video-studio", async (req, res) => {  
  try {  
    ...
    if (mode === "image2video") {  
      ...
      const baseInput = {  
        prompt: vprompt, size, duration: secs, negative_prompt: "text, logo, watermark, letters, subtitles", enable_prompt_expansion: true,  
      };  
      ...
      const imageVariants = [ { key: "start_image", value: incomingImage }, { key: "image", value: incomingImage }, { key: "input_image", value: incomingImage }, ];  
...
/* ====================== (ADDED) FFMPEG TRIM UTILS & ROUTE ====================== */
// Tiny helper: call ffmpeg-static safely
...
// POST /api/video-2s  (multipart/form-data; field "file")
// Returns: { ok:true, url:"https://.../uploads/<trimmed>.mp4" }
