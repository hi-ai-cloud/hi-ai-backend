...
// Express + OpenAI + Replicate (polling, data:URL safe, model routing)
// КОРОТКИЕ URL: /uploads/..., без data: в query. Есть /api/shorten для шортлинка.
...
// ВАЖНО: без express.raw({type:"*/*"}) — он ломает multer!
// Нормальные парсеры:
...
// Раздача /uploads с нужными заголовками (чтобы можно было рисовать на canvas)
...
// Multer (после парсеров json/urlencoded, но это ок — multer читает свой поток сам)
const upload = multer({
...
function makeDataUrlSafe(dataUrl) {
  ...
  if (!/data:[^;]+;base64,/.test(s)) return `data:image/png;base64,${s.replace(/^data:[^,]+,/, "")}`;
  return s;
}
...
// ------------------- SHORTEN (серверный шортлинг) -------------------
app.post("/api/shorten", express.json(), async (req, res) => {
  try {
    const { url } = req.body || {};
    if (!url || typeof url !== "string") return res.status(400).json({ ok:false, error: "bad_url" });
    const r = await fetch("https://tinyurl.com/api-create.php?url=" + encodeURIComponent(url));
    if (!r.ok) throw new Error("tinyurl fail " + r.status);
    const short = (await r.text()).trim();
    return res.json({ ok:true, short });
  } catch (e) {
    console.error("shorten error:", e);
    return res.status(502).json({ ok:false, error: "shorten_failed" });
  }
});
...
app.get("/api/proxy", async (req, res) => {
  ...
  try {
    const origin = (process.env.PUBLIC_ORIGIN && process.env.PUBLIC_ORIGIN.replace(/\/+$/, "")) || absoluteOrigin(req);
    let upstream = await fetch(u, {
      redirect: "follow",
      headers: {
        "Referer": origin + "/",
    ...
    if (!upstream.ok) {
      const retry = await fetch(u, { redirect: "follow" });
...
async function replicateCreate(version, input) {
  ...
  return fetchJson("https://api.replicate.com/v1/predictions", {
    method: "POST", headers: REPLICATE_HEADERS(), body: JSON.stringify({ version, input })
  });
}
async function replicateCreateBySlug(slug, input) {
  ...
  return fetchJson(`https://api.replicate.com/v1/models/${slug}/predictions`, {
    method: "POST", headers: REPLICATE_HEADERS(), body: JSON.stringify({ input })
  });
}
async function pollPredictionByUrl(getUrl, { tries = 240, delayMs = 1500 } = {}) {
  ...
  for (let i=0;i<tries;i++){
    ...
    if (last.status === "failed" || last.status === "canceled") throw new Error(`Replicate failed: ${last?.error || last?.status || last?.logs || "unknown"}`);
    await sleep(delayMs);
  }
...
app.post("/", (req, res) => {
  ...
  if (imageActions.has(action)) { req.url = "/api/image-studio"; app._router.handle(req, res); }
  else if (videoActions.has(action)) { req.url = "/api/video-studio"; app._router.handle(req, res); }
  else { req.url = "/api/brand-post"; app._router.handle(req, res); }
});
...
Return STRICT JSON:
{
  ...
  "visual_prompt": "clean prompt for image generator without text in image"
      ...
      try {
        ...
        async function tryFluxByVersion() {
          ...
          const job = await replicatePredict(process.env.REPLICATE_MODEL_VERSION_FLUX, {
            prompt: vprompt, go_fast:false, megapixels:"1", prompt_strength:0.85, num_outputs:1, output_format:"png", output_quality:90
          });
        ...
        async function trySDXLByVersion() {
          ...
          const job = await replicatePredict(process.env.REPLICATE_MODEL_VERSION_SDXL, {
            prompt: vprompt, negative_prompt: negative, num_inference_steps:30, guidance_scale:7.0, scheduler:"DPMSolverMultistep", num_outputs:1
          });
          return Array.isArray(job.output) ? job.output[0] : job.output;
        }
        async function tryFluxBySlug() {
          ...
          const job = await replicateCreateBySlug("black-forest-labs/flux-schnell", {
            prompt: vprompt, aspect_ratio: ratio.replace("-", ":"), num_outputs:1, output_format:"png", output_quality:90
          });
        ...
        try {
          if (chooseImageModelKey({idea,style,hint:imageModelHint}) === "flux" && process.env.REPLICATE_MODEL_VERSION_FLUX) image_url = await tryFluxByVersion();
          else if (process.env.REPLICATE_MODEL_VERSION_SDXL) image_url = await trySDXLByVersion();
        } catch (e) { modelError = String(e); }
    ...
    return res.json({
      ...
      vprompt,
      image_url,
      model_used: (chooseImageModelKey({idea,style,hint:imageModelHint}) || "default").toUpperCase(),
  ...
  try {
    ...
    async function runSingle({ prompt, image_data, mask_data, strength, seed }) {
      ...
      if (action === "text2img") {
        ...
        const SET = action === "remove_bg" ? REMOVE_BG_MODELS : UPSCALE_MODELS;
    ...
    if (batch_count === 1) {
      const anglePhrase = (camera_path === "orbit") ? "front view" : (angles[0] || "");
    ...
    const baseSeed = seed != null ? Number(seed) : Math.floor(Math.random()*1e7);
    const seqAngles = angles.length ? angles : ["front view","3/4 angle, left side","side view","3/4 angle, right side"];

    const tasks = [];
    for (let i = 0; i < count; i++) {
      const anglePhrase = seqAngles[i % seqAngles.length] || "";
...
app.post("/api/video-studio", async (req, res) => {
  try {
    ...
    const vprompt = buildVPrompt({ idea, style, ratio, seconds: secs, category: body.category, subcategory: body.subcategory });
    ...
    if (mode === "image2video" && !incomingImage) return res.json({ ok:false, error:"Provide 'image_url' or 'image_data_url' (or 'image')" });
    ...
    const videoSlug = String(body.video_model_slug || body.replicate_video_slug || body.REPLICATE_MODEL_SLUG_VIDEO || "").trim();

    const wanSeconds = secs;
    const wanSize = ((r)=> (r==="9:16"?"1080*1920": r==="16:9"?"1920*1080":"1080*1080"))(ratio);

    const imageModelKey = chooseImageModelKey({ idea, style, hint: (body.image_model_hint || "auto").toString().toLowerCase() });
    ...
    const wantsEmoji = !!optsIn.emojis;
    const wantsHash  = !!optsIn.auto_hashtags;

    const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
    let caption = null, vpromptOut = vprompt, gptUsed = false;
    if (!image_only) {
      try {
        const user = `
...
Constraints:
- Tone: ${body.preset || "neutral"}
- CTA: ${body.cta || "Learn more"}
- Category: ${body.category || "General"}${body.subcategory ? " / " + body.subcategory : ""}
...
Idea: "${idea}"
Return JSON: { "caption":"...", "visual_prompt":"..." }
`.trim();
        ...
        vpromptOut = (j.visual_prompt || vprompt).toString().trim();
        if (!caption) caption = `✨ ${idea}\n Learn more and take action today.\n\n➡️ Learn more\n\nhttps://hi-ai.ai #ai #automation #creativity`.slice(0,220);
        gptUsed = true;
      } catch {}
    }

    if (text_only) {
      return res.json({ ok:true, caption, vprompt:vpromptOut, video_url:null, image_url:null, gpt_used:!!gptUsed, ratio, seconds:wanSeconds, size_used:wanSize, mode:"text_only" });
    }
    ...
    if (force_video) {
      ...
      if (videoSlug) {
        try {
          const job = await replicateCreateBySlug(videoSlug, { prompt: vpromptOut, size: wanSize, duration: wanSeconds,
      ...
      if (!video_url && process.env.REPLICATE_MODEL_VERSION_VIDEO) {
        try {
          const job = await replicatePredict(process.env.REPLICATE_MODEL_VERSION_VIDEO, { prompt: vpromptOut, size:wanSize, duration:wanSeconds,
            negative_prompt:"text, logo, watermark, letters, subtitles", enable_prompt_expansion:true });
          const out = job?.output;
          video_url = typeof out === "string" ? out : Array.isArray(out) ? out[0] : null;
        } catch {}
      }
      if (!video_url) return res.status(502).json({ ok:false, error:"Video generation failed (forced). Check model slug/version logs." });
      return res.json({ ok:true, caption:null, vprompt:vpromptOut, video_url, image_url:null, gpt_used:!!gptUsed, ratio, seconds:wanSeconds, size_used:wanSize, mode:"video" });
    }

    // IMAGE ONLY
    if (image_only && optsIn.image !== false) {
      let image_url = null;
      ...
      if (versionImg) {
        const inputI = imageModelKey === "flux"
          ? { prompt: vpromptOut, go_fast:false, megapixels:"1", num_outputs:1, output_format:"png", output_quality:90 }
          : { prompt: vpromptOut, width:w, height:h, num_inference_steps:30, guidance_scale:7.0, num_outputs:1 };
        try {
      ...
      return res.json({ ok:true, caption, vprompt:vpromptOut, video_url:null, image_url: image_url || null, gpt_used:!!gptUsed, ratio, seconds:wanSeconds, size_used:wanSize, mode:"image_only" });
    }

    // NORMAL: video then fallback to image
    ...
    if (!video_url && (optsIn.image !== false)) {
      const versionImg = imageModelKey === "flux" ? process.env.REPLICATE_MODEL_VERSION_FLUX : process.env.REPLICATE_MODEL_VERSION_SDXL;
      if (versionImg) {
        const inputI = imageModelKey === "flux"
          ? { prompt: vpromptOut, go_fast:false, megapixels:"1", num_outputs:1, output_format:"png", output_quality:90 }
    ...
    return res.json({ ok:true, caption, vprompt:vpromptOut, video_url: video_url || null, image_url: image_url || null, gpt_used:!!gptUsed, ratio, seconds:wanSeconds, size_used:wanSize, mode: video_url ? "video" : image_url ? "image_fallback" : "text_only" });
  } catch (e) {
    res.status(500).json({ ok:false, error:String(e.message || e) });
  }
});
