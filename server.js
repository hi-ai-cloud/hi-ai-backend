...
try {
  ...
  console.warn("[KEYS] keys.json not found or invalid, starting with empty map:", e.message || e);
  KEYS = {};
}

// На всякий случай — бутстрапим TEST100, если его нет
if (!KEYS.TEST100) {
  console.warn("[KEYS] TEST100 not found, injecting DEV test key");
  KEYS.TEST100 = {
    max: 100,
    used: 0,
  ...
  try {
    fs.mkdirSync(path.dirname(KEYS_DB), { recursive: true });
    fs.writeFileSync(KEYS_DB, JSON.stringify(KEYS, null, 2));
    console.log("[KEYS] keys.json written with TEST100");
  } catch (e) {
    console.error("[KEYS] Failed to write keys.json:", e);
  }
}
