...
if (!fs.existsSync(SHORT_DB)) fs.writeFileSync(SHORT_DB, JSON.stringify({ map: {} }, null, 2));

const readShortDB = () => {
  try { return JSON.parse(fs.readFileSync(SHORT_DB, "utf8")); } catch { return { map: {} }; }
};
const writeShortDB = (obj) => {
  const tmp = SHORT_DB + ".tmp";
  fs.writeFileSync(tmp, JSON.stringify(obj, null, 2));
  fs.renameSync(tmp, SHORT_DB);
};
const b62 = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
const rnd = (n=7)=> Array.from({length:n},()=> b62[Math.floor(Math.random()*b62.length)]).join("");
...
app.post("/api/shorten", async (req, res) => {
  try {
    ...
    const db = readShortDB();
    const existing = Object.entries(db.map).find(([,v]) => v === url);
    let code = existing ? existing[0] : "";
    if (!code) { do { code = rnd(7); } while (db.map[code]); db.map[code] = url; writeShortDB(db); }

    const short = `${absoluteOrigin(req)}/s/${code}`;
    res.setHeader("Access-Control-Allow-Origin", "*");
    res.json({ ok:true, short });
  } catch (e) {
    console.error("shorten failed:", e);
    res.status(500).json({ ok:false, error:"shorten_failed" });
  }
});

app.get("/s/:code", (req, res) => {
  const db = readShortDB();
  const url = db.map[String(req.params.code || "")];
